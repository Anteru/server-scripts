...
def Process (config):
	...
	cutoffTime = processTime - datetime.timedelta (days=int (config ['options']['min_age']))
	...
	typ, data = conn.uid ('SEARCH', '(BEFORE {})'.format (cutoffTime.strftime ("%d-%b-%Y")))
	if data != [b'']:
		uids = data[0].split ()
		# Back them up first
		if config ['backup']['type'] == 'Maildir':
			...
			for uid in uids:
				rv, maildata = conn.uid ('FETCH', uid, '(RFC822)')
